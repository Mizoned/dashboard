export default {
    ...
    mounted(el, binding) {
        ...
        let parent = binding.value.parentSelector ? el.closest(binding.value.parentSelector) : el.parentElement;
        if (!parent) {
            throw new Error(`Родительский элемент с селектором ${binding.value.parentSelector} не определен`)
        }
